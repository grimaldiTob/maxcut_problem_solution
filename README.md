...
Benchmarking classical and quantum solvers for the Maximum Cut problem on several graph families of sizes $n \in \{8, 10, 12, 14, 18, 20, 24, 30, 36\}$.
...
- **3-Regular**: a graph where every node has a degree of exactly 3.
- **Erdòs-Rényi**: random graphs with a target average density and guaranteed global connectivity. The average node degree fluctuates within the $\pm 25\%$ of a target degree.
...
- `data/graphs/`: graph instances in JSON edge-list format are stored here, divided by `graph_family`.
- `src/common/`: graph generators, Hamiltonian builders, and cut plotting utilities.
- `src/classic/`: classical implementations.
...
- **QAOA (p = 2)** shows better performance result and more guarantees to find the best solution with an additional cost/mixer hamiltonians added in the ansatz, but it also increases the exection time.
